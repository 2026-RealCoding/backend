...
## Q1. 세션 방식 대신 JWT(JSON Web Token)로 인증을 구현하는 이유로 가장 적절한 것은?
- A) JWT는 세션보다 암호화 강도가 높아 보안이 우수하기 때문이다
- B) 서버가 세션 상태를 저장하지 않아도 되는 Stateless 방식이라, 서버 확장(Scale-out)이 쉽고 여러 서버에서 공유가 필요 없기 때문이다
- C) JWT는 브라우저의 쿠키 저장 용량 제한을 받지 않기 때문이다
- D) JWT는 Spring Security 없이는 사용할 수 없어 보안성이 보장되기 때문이다
- E) 세션은 모바일 앱에서 동작하지 않기 때문이다

**정답:** B
**해설:** JWT의 가장 큰 장점은 **Stateless(무상태)** 방식이라는 점이다. 서버가 세션을 저장하지 않으므로 서버를 여러 대로 확장해도 세션 공유 문제가 없고, 토큰 자체에 사용자 정보가 담겨 있어 인증 상태를 서버 간에 공유할 필요가 없다. 암호화가 아닌 서명(Signature)으로 위변조만 검증한다.

---

## Q2. Spring 6.1에서 RestClient가 새로 도입되어 권장되는 이유로 가장 적절한 것은?
- A) RestTemplate보다 훨씬 빠른 성능을 제공하기 때문이다
- B) RestTemplate은 유지보수 모드이고, RestClient는 WebClient와 유사한 간결한 Fluent API를 동기 방식으로 제공하기 때문이다
- C) RestClient만 JSON 응답을 파싱할 수 있기 때문이다
- D) RestTemplate은 Spring Boot 4에서 완전히 삭제되었기 때문이다
- E) RestClient는 비동기 처리를 지원하기 때문이다

**정답:** B
**해설:** RestTemplate은 유지보수 모드(새 기능 추가 없음)이며, WebClient는 비동기/리액티브 방식이다. RestClient는 Spring 6.1에서 도입된 **동기 HTTP 클라이언트**로, WebClient와 비슷한 Fluent API(체이닝 방식)를 제공하면서도 동기 코드로 작성할 수 있어 가장 권장되는 선택이다.
...
## Q4. 수동 페이징(stream().skip().limit())과 비교했을 때 Spring Data JPA의 Pageable이 효율적인 이유로 가장 적절한 것은?
...
## Q5. Spring Boot에서 외부 API를 호출할 때 사용하는 동기 HTTP 클라이언트로, Spring 6.1에서 도입되어 현재 권장되는 것은?
- A) WebClient
- B) RestTemplate
- C) RestClient
- D) HttpClient
...
## Q6. 네이버 쇼핑 검색 API를 호출할 때 인증을 위해 HTTP 요청 헤더에 포함해야 하는 것은?
...
## Q7. JWT(JSON Web Token)의 구조로 올바른 것은?
...
## Q8. JWT의 Payload에 대한 설명으로 올바른 것은?
...
## Q9. Spring Data JPA에서 Pageable을 사용할 때, 페이지 번호는 몇부터 시작하는가?
- A) -1
- B) 0
- C) 1
...
## Q10. 다음 코드에서 환경변수 NAVER_CLIENT_ID가 설정되지 않았을 때 clientId에 주입되는 값은?
...
## Q11. 다음 JWT 검증 코드에서 validateToken이 false를 반환하는 경우가 아닌 것은?
...
## Q12. 다음 Controller 코드에서 GET /users?page=1&size=5&sort=name,asc 요청 시 실행되는 SQL로 가장 적절한 것은?
