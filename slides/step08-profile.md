...
gradlew.bat bootRun --args="--spring.profiles.active=prod"  # Windows

# 3. 환경 변수
export SPRING_PROFILES_ACTIVE=prod    # macOS / Linux
...
# .env 파일로 민감 정보 분리 (1/2)

**문제:** API 키, 비밀번호 같은 민감 정보를 어떻게 관리하지?

| 항목 | `application.properties` | `.env` |
|------|--------------------------|--------|
| **Git 커밋** | O (공개 OK) | **X (절대 금지)** |
| **용도** | 공통 설정, 로직 | 민감 정보, 로컬 전용 |
...
- `optional:` — 파일이 없어도 에러 없음
- `[.properties]` — `.env`를 properties 형식으로 파싱

---

# .env 파일로 민감 정보 분리 (2/2)

```
프로젝트 루트/
├── .env.example    ← Git 커밋 O (템플릿)
├── .env            ← Git 커밋 X (실제 값)
└── .gitignore      ← .env 제외
```
...
> `.env.example`은 **"어떤 키가 필요한가"** 알려주는 체크리스트 역할!
