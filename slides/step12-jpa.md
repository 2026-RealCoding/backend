...
public class UserEntity {
    ...
    @Id                                    // 2. @Id로 기본키 지정
    @GeneratedValue(strategy = IDENTITY)   // 3. 데이터베이스가 자동으로 ID 번호를 부여하는 전략
    private Long id;
...
> 지금 이해하지 못해도 괜찮다. 나중에 다시 만나게 된다. (@Transactional의 세부 동작)
...
Dirty Checking이란 JPA가 엔티티의 변경을 자동으로 감지하여 UPDATE 쿼리를 실행하는 기능이다.

> 지금 이해하지 못해도 괜찮다. 나중에 다시 만나게 된다. (Dirty Checking, Lazy Loading)
...
```properties
# SQLite 파일 기반 DB
# Windows에서도 동일 경로 사용 (Gradle이 자동 처리)
