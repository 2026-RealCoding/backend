...
public class UserController {
    // ConcurrentHashMap: 여러 요청이 동시에 와도 안전하게 데이터를 저장하는 Map
    private final Map<Long, User> store = new ConcurrentHashMap<>();
    // AtomicLong: 동시 요청에도 안전하게 숫자를 증가시키는 도구
...
public interface UserRepository {
    List<User> findAll();
    Optional<User> findById(Long id);  // Optional: 값이 있을 수도, 없을 수도 있는 컨테이너
    User save(User user);
...
> 지금 이해하지 못해도 괜찮다. 나중에 다시 만나게 된다. (Optional, Stream API)
...
이것이 **DI(Dependency Injection, 의존성 주입)** 를 통한 **의존성 역전 원칙**
